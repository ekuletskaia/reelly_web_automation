...
Feature: Off-plan tests
  ...
  @mobile_emulator
  Scenario: Mobile_emulator: User can filter the off plan products by Unit price range
    When Tap on “off plan” at the left side menu
    Then Verify the title Total projects in the page
    When Set the Filter the products by price range from 1200000 to 2000000 AED
    Then Verify the price in all cards is inside the range (1200000 - 2000000)